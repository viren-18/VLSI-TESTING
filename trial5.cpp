...
void SerialFaultSimulation(Module File, set<pair<string, string>> faultList, vector<string> &testvector)
{
    for (auto it : testvector)
    {
        ...
        for(int v=0;v<File.gates.size();v++){
            ...
            cout<<"Gate -"<<" "<<File.gates[v].g_name<<" "<<"Output -"<<" "<<File.gates[v].outputs[0]<<" "<<"Value ="
            <<" "<<mp[File.gates[v].outputs[0]];
            cout<<endl;
        }
    ...
    // for (auto it : faultList)
    // {
    //     string fault_name = it.first;
    //     int fault_val = (int)(it.second[it.second.size() - 1]) - '0';
...
// void multiplefault(unordered_map<string, int> true_value, unordered_map<string, int> mp, set<pair<string, string>> faultList)
// {
//     for(auto it : faultList)
//     {
...
int main()
{
    ...
    // cout << "Enter the fault name and val" << endl;
    // string temp;
    // int val;
    // cin >> temp >> val;
    // for(auto it : module.outputs)
    ...
    // SerialFaultSimulation(temp, val, module, mp2);
